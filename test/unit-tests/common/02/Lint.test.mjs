...
describe('lint plugin', function () {
  it('finds incomplete documentation', function () {
    const lintJSON = generatedDocumentationFiles['lint.json'];
    
    // Compare two arrays to find out which one is missing documentation
    const existingParams = collect(lintJSON)
      .where('name', '===', 'incompleteArrowFunctionDocumentation')
      .firstOrFail()
      .codeParams;
    const documentedParams = collect(lintJSON)
      .where('name', '===', 'incompleteArrowFunctionDocumentation')
      .firstOrFail()
      .docParams;
    const undocumentedParam = collect(existingParams).diff(documentedParams).firstOrFail();
